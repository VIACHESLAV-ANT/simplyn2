{
const pilotItems = document.querySelectorAll('.review__button');

// Отримуємо всі елементи <li> з класом "products__item"
const productItems = document.querySelectorAll('.review__item');
...
function handlePilotItemClick(index) {
  ...
  pilotItems.forEach(function(item) {
    item.classList.remove('review__button--active');
  });
  // Додаємо клас "--active" лише до обраного елемента "pilot"
  pilotItems[index].classList.add('review__button--active');
  ...
  productItems[index].classList.add('review__item--active');
}
